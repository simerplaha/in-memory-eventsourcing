...
object StepWise {
  ...
  private final case class MultiMessage(timeout: FiniteDuration, f: (Seq[Any], Any) ⇒ Any, trace: Trace) extends AST
  ...
  final case class Steps[T, U](ops: List[AST], keepTraces: Boolean) {
    ...
    def expectMessageKeep(timeout: FiniteDuration)(f: (T, U) ⇒ Unit): Steps[T, U] =
      copy(ops = Message(timeout, (msg, value) ⇒ {
    ...
    def expectMultipleMessagesKeep(timeout: FiniteDuration, count: Int)(f: (Seq[T], U) ⇒ Unit): Steps[T, U] =
      copy(ops = MultiMessage(timeout, (msgs, value) ⇒ {
    ...
    def expectTerminationKeep(timeout: FiniteDuration)(f: (Terminated, U) ⇒ Unit): Steps[T, U] =
      copy(ops = Termination(timeout, (t, value) ⇒ {
  ...
  private def run[T](ctx: ActorContext[T], ops: List[AST], value: Any): Behavior[T] =
    ops match {
      ...
      case MultiMessage(t, f, trace) :: tail ⇒
        val expectedCount = value.asInstanceOf[(Any, Int)]._2
        val deadline = Deadline.now + t
        def behavior(count: Int, acc: List[Any]): Behavior[T] = {
          ctx.setReceiveTimeout(deadline.timeLeft)
          Full {
            case Sig(_, ReceiveTimeout) ⇒ throwTimeout(trace, s"timeout of $t expired while waiting for $expectedCount messages (got only $count)")
            case Msg(_, msg) ⇒
              val nextCount = count + 1
              if (nextCount == expectedCount) {
                run(ctx, tail, f((msg :: acc).reverse, value))
              } else
                behavior(nextCount, msg :: acc)
            case Sig(_, other) ⇒ throwIllegalState(trace, s"unexpected $other while waiting for $expectedCount messages (got $count valid ones)")
          }
        }
        behavior(0, Nil)
